...
class V2RegressionTests(unittest.TestCase):
    ...
    def test_fetch_detail_bundle_queries_announcement_with_album_id(self):
        class AlbumAnnouncementClient(scraper.JDClient):
            def __init__(self):
                self.calls = []

            def api(self, function_id, body, *args, **kwargs):
                self.calls.append((function_id, body))
                if function_id == "getWareCoreDataBff":
                    return {"data": {"basicData": {"albumId": 5525607}}}
                if function_id == "getPaimaiRealTimeData":
                    return {"data": {}}
                if function_id == "getProductBasicInfo":
                    return {"data": {}}
                if function_id == "queryProductDescription":
                    return {"data": "description"}
                if function_id == "queryNotice":
                    return {"data": "notice"}
                if function_id == "queryAnnouncement":
                    if body.get("albumId") == 5525607:
                        return {"data": {"content": "auction announcement"}}
                    return {"data": {"content": ""}}
                if function_id == "queryAttachFilesForIntro":
                    return {"data": []}
                raise AssertionError(f"Unexpected API call: {function_id}")

        client = AlbumAnnouncementClient()

        bundle = client.fetch_detail_bundle("310826968", {})

        self.assertEqual(bundle["announcement_html"], "auction announcement")
        announcement_calls = [body for function_id, body in client.calls if function_id == "queryAnnouncement"]
        self.assertEqual(len(announcement_calls), 1)
        self.assertEqual(announcement_calls[0].get("albumId"), 5525607)
    ...
    def test_ai_context_keeps_activity_time_hints_and_truncates_text(self):
        ...
        self.assertLessEqual(len(context.detail_text), scraper.AI_DETAIL_TEXT_LIMIT)
        self.assertLessEqual(len(context.notice_text), scraper.AI_NOTICE_TEXT_LIMIT)
    ...
    def test_ai_batch_failure_does_not_fall_back_to_single_fields_by_default(self):
        class FailingClient:
        ...
        extractor = AIFieldExtractor(max_retries=0, enable_single_field_fallback=False)
        ...
        extractor.extract_field = fake_single
        results = extractor.batch_extract(
            [("contact_info", "联系方式", "联系人和电话")],
            AIExtractionContext(
                html_key_values={},
                detail_text="",
                notice_text="",
        ...
        self.assertIsNone(results["contact_info"].value)

    def test_ai_circuit_breaker_skips_calls_after_repeated_failures(self):
        ...
        extractor = AIFieldExtractor(
            max_retries=0,
            enable_single_field_fallback=False,
        ...
        extractor.batch_extract([("contact_info", "联系方式", "联系人和电话")], context)
        extractor.batch_extract([("special_notice", "特别告知", "提示事项")], context)

        self.assertEqual(client.calls, 1)

    def test_ai_timeout_zero_means_no_requests_timeout(self):
        from jd.ai_extractor import BaseAIClient

        client = BaseAIClient(api_key="test", base_url="https://example.test", timeout=0)

        self.assertIsNone(client.timeout)

    def test_init_ai_extractor_uses_config_api_key_when_arguments_are_empty(self):
        old_api_key = scraper.cfg.ai.api_key
        old_model = scraper.cfg.ai.model
        old_model_name = scraper.cfg.ai.model_name
        old_base_url = scraper.cfg.ai.base_url
        old_create = scraper.create_ai_extractor
        old_extractor = scraper.ai_extractor
        captured = {}

        def fake_create_ai_extractor(config):
            captured.update(config)
            return object()

        try:
            scraper.cfg.ai.api_key = "config-key"
            scraper.cfg.ai.model = "qwen"
            scraper.cfg.ai.model_name = "qwen-plus-test"
            scraper.cfg.ai.base_url = "https://example.test"
            scraper.create_ai_extractor = fake_create_ai_extractor
            scraper.ai_extractor = None

            scraper.init_ai_extractor("", "", "")

            self.assertEqual(captured["api_key"], "config-key")
            self.assertEqual(captured["provider"], "qwen")
            self.assertEqual(captured["model_name"], "qwen-plus-test")
            self.assertEqual(captured["base_url"], "https://example.test")
            self.assertIsNotNone(scraper.ai_extractor)
        finally:
            scraper.cfg.ai.api_key = old_api_key
            scraper.cfg.ai.model = old_model
            scraper.cfg.ai.model_name = old_model_name
            scraper.cfg.ai.base_url = old_base_url
            scraper.create_ai_extractor = old_create
            scraper.ai_extractor = old_extractor

    def test_ai_circuit_breaker_can_be_disabled_for_quality_mode(self):
        class CountingFailingClient:
            def __init__(self):
                self.calls = 0
    ...
    def test_ai_batch_budget_limits_one_batch_per_item(self):
        old_max_batches = scraper.cfg.ai.max_batches_per_item
        old_extractor = scraper.ai_extractor
        paimai_id = "budget-item"
        try:
            scraper.cfg.ai.max_batches_per_item = 1
            scraper.ai_extractor = FakeBatchAIExtractor(
                {
                    "contact_info": "18800000000",
                    "special_notice": "risk text",
                }
            )
            parsed = scraper.ParsedHTML(key_values={}, text="", rows=[])
            scraper.start_ai_batch_budget(paimai_id)

            first = scraper.ai_batch_extract_fields(
            ...
            second = scraper.ai_batch_extract_fields(
                [("special_notice", "notice", "risk notice")],
                parsed=parsed,
                notice_parsed=parsed,
                asset_group="real_estate",
                paimai_id=paimai_id,
            ...
            self.assertEqual(len(scraper.ai_extractor.calls), 1)
        finally:
            scraper.clear_ai_batch_budget(paimai_id)
            scraper.cfg.ai.max_batches_per_item = old_max_batches
            scraper.ai_extractor = old_extractor

    def test_combined_ai_prefetch_fetches_common_and_special_in_one_call(self):
        old_max_batches = scraper.cfg.ai.max_batches_per_item
        old_extractor = scraper.ai_extractor
        paimai_id = "combined-ai-item"
        try:
            scraper.cfg.ai.max_batches_per_item = 1
            scraper.ai_extractor = FakeBatchAIExtractor(
                {
                    "contact_info": "18800000000",
                    "building_area": "120平方米",
                }
            )
            parsed = scraper.ParsedHTML(key_values={}, text="", rows=[])
            scraper.start_ai_batch_budget(paimai_id)

            results = scraper.prefetch_combined_ai_results(
                asset_group="real_estate",
                parsed=parsed,
                notice_parsed=parsed,
                paimai_id=paimai_id,
            ...
            self.assertIn("building_area", results)
            self.assertEqual(len(scraper.ai_extractor.calls), 1)
        finally:
            scraper.clear_ai_batch_budget(paimai_id)
            scraper.cfg.ai.max_batches_per_item = old_max_batches
    ...
    def test_v2_formal_schema_keeps_display_amount_and_typed_extraction_columns(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "formal_schema.sqlite"
            db = scraper.JDScraperDatabase(db_path)
            db.init_schema()

            conn = sqlite3.connect(db_path)
            try:
                common_columns = {row[1] for row in conn.execute("PRAGMA table_info(auction_items_common)")}
                extraction_columns = {row[1] for row in conn.execute("PRAGMA table_info(field_extractions)")}
            finally:
                conn.close()
        ...
        for column in (
            ...
            self.assertIn(column, common_columns)
        for column in ("value_type", "numeric_value", "date_value", "datetime_value"):
            self.assertIn(column, extraction_columns)

        schema = "\n".join(MYSQL_SCHEMA)
        self.assertIn("disposal_agency", schema)
        self.assertIn("current_price_amount DECIMAL(18,2)", schema)
        self.assertIn("numeric_value DECIMAL(18,2)", schema)
        self.assertIn("datetime_value DATETIME", schema)

    def test_v2_common_upsert_populates_display_current_and_typed_evidence(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "formal_values.sqlite"
            db = scraper.JDScraperDatabase(db_path)
            db.init_schema()
            db.seed_field_catalog()
            ...
            values = {
                "asset_type": "房地产",
                "project_name": "测试项目",
                "asset_location": "北京市朝阳区",
                "signup_start_time": "2026年6月30日9时起",
                "signup_end_time": "2026年7月1日9时止",
                "disposal_party": "测试处置机构",
                "start_price_raw": "2,200,000元",
                "final_price_raw": "3,142,750元",
            }
            field_results = {
                "signup_start_time": {"status": "extracted", "method": "unit_test"},
                "start_price_raw": {"status": "extracted", "method": "unit_test"},
            }
            ...
            db.upsert_common_item(
                ...
                special_values={"right_certificate_no": "X1"},
            )
            ...
            try:
                common = conn.execute(
                    """
                    SELECT disposal_agency, start_price_display, current_price_display,
                           current_price_amount, final_price_display
                    FROM auction_items_common
                    WHERE paimai_id='unit-1'
                    """
                ).fetchone()
                start_evidence = conn.execute(
                    """
                    SELECT value_type, numeric_value
                    FROM field_extractions
                    WHERE paimai_id='unit-1'
                      AND field_namespace='common'
                      AND field_key='start_price_raw'
                    """
                ).fetchone()
                time_evidence = conn.execute(
                    """
                    SELECT value_type, datetime_value
                    FROM field_extractions
                    WHERE paimai_id='unit-1'
                      AND field_namespace='common'
                      AND field_key='signup_start_time'
                    """
                ).fetchone()
            finally:
                conn.close()

            self.assertEqual(common["disposal_agency"], "测试处置机构")
            self.assertEqual(common["start_price_display"], "2,200,000元")
            self.assertEqual(common["final_price_display"], "3,142,750元")
            self.assertEqual(common["current_price_display"], "3,142,750元")
            self.assertEqual(common["current_price_amount"], 3142750)
            self.assertEqual(start_evidence["value_type"], "money")
            self.assertEqual(start_evidence["numeric_value"], 2200000)
            self.assertEqual(time_evidence["value_type"], "datetime")
            self.assertEqual(time_evidence["datetime_value"], "2026-06-30 09:00:00")

    def test_v2_mysql_direct_writer_adapter_exists(self):
        self.assertTrue(hasattr(jd_mysql_store, "MySQLJDScraperDatabase"))
    ...
    def test_v2_ip_fields_fallback_to_project_title_when_detail_has_images_only(self):
        ...
        self.assertEqual(values["ip_type"], "著作权、专利权")
    ...
    def test_v2_ip_details_use_vision_rows_from_image_tables_before_title_summary(self):
        old_vision_enabled = scraper.cfg.ai.enable_vision_ai
        scraper.cfg.ai.enable_vision_ai = True
        ...
        try:
            ...
            scraper.cfg.ai.enable_vision_ai = old_vision_enabled
    ...
    def test_v2_real_estate_rental_fields_fall_back_from_announcement_text(self):
        scraper.ai_extractor = FakeBatchAIExtractor({})
        notice_parsed = extract_key_values_from_html(
            "<p>标的名称：无锡市滨湖区建筑路672-674号（1F）-三年租赁权。</p>"
            "<p>坐落：无锡市滨湖区建筑路672-674号（1F） 现状：在租 2.租赁信息 "
            "租赁面积（㎡）：126.55（纯一楼） 租赁期限（年）：3 免租期（月）：0 "
            "租赁底价（元/年）：144,000 允许从事行业：服务业、零售业 竞拍时间：2026年7月1日9:00至15:00</p>"
        )
        ...
        values, _, _ = extract_special_values(
            ...
            parsed=extract_key_values_from_html(""),
            notice_parsed=notice_parsed,
            core={"data": {"basicData": {"title": "房屋租赁权"}}},
            paimai_id="test-rental-real-estate",
            attachments={"files": [], "media": []},
        )

        self.assertEqual(values["building_area"], "126.55㎡")
        self.assertEqual(values["use_term"], "3年")
        self.assertEqual(values["property_status"], "在租")
        self.assertEqual(values["property_use"], "服务业、零售业")
    ...
    def test_v2_ai_context_focuses_multi_item_notice_table_on_current_subject(self):
        notice_parsed = ParsedHTML(
            key_values={},
            text="变卖标的表格包含多个车位。",
            rows=[
                ["序号", "标的物", "产权证号", "建筑面积（m²）", "评估价（元）", "变卖起拍价（元）"],
                ["1", "德庆县龙母大街(龙湖东岸)聚龙湖畔地下室 A29号车位", "粤(2022)德庆县不动产权第 0004213号", "39.94", "81800", "46000"],
                ["2", "德庆县龙母大街(龙湖东岸)聚龙湖畔地下室 A32号车位", "粤(2022)德庆县不动产权第 0004181号", "25.06", "81800", "46000"],
                ["123", "德庆县龙母大街(龙湖东岸)聚龙湖畔地下室 158号摩托车位", "粤(2022)德庆县不动产权第 004313号", "4.99", "8100", "5000"],
            ],
        )
        ...
        focused = context.html_key_values.get("_target_item_table_rows_json", "")
        self.assertIn("158号摩托车位", focused)
        self.assertIn("004313号", focused)
        self.assertIn("4.99", focused)
        self.assertNotIn("0004213号", focused)
        self.assertNotIn("39.94", focused)

    def test_v2_ai_context_keeps_late_notice_table_rows_for_large_multi_item_pages(self):
        rows = [["序号", "标的物", "产权证号", "建筑面积（m²）", "评估价（元）", "变卖起拍价（元）"]]
        for index in range(1, 131):
            name = f"德庆县龙母大街(龙湖东岸)聚龙湖畔地下室 {index}号摩托车位"
            cert = f"粤(2022)德庆县不动产权第 {index:06d}号"
            area = "4.99"
            assess = "8200"
            start = "5000"
            if index == 123:
                name = "德庆县龙母大街(龙湖东岸)聚龙湖畔地下室 158号摩托车位"
                cert = "粤(2022)德庆县不动产权第 004313号"
                assess = "8100"
            rows.append([str(index), name, cert, area, assess, start])

        context = scraper.build_ai_context(
            extract_key_values_from_html(""),
            ParsedHTML(key_values={}, text="变卖标的表格包含多个车位。", rows=rows),
            "real_estate",
            "310700740",
            project_name="德庆县龙母大街(龙湖东岸)聚龙湖畔地下室 158号摩托车位",
        )

        notice_rows = context.html_key_values.get("_notice_table_rows_json", "")
        focused_rows = context.html_key_values.get("_target_item_table_rows_json", "")
        self.assertIn("158号摩托车位", notice_rows)
        self.assertIn("004313号", notice_rows)
        self.assertIn("158号摩托车位", focused_rows)
        self.assertNotIn("000001号", focused_rows)

    def test_v2_ai_context_keeps_long_notice_text_with_late_activity_information(self):
        late_sentence = "本次竞价时间：2026年9月18日10时起至2026年9月19日10时止。"
        notice_text = "公告前言。" + ("普通说明。" * 900) + late_sentence

        context = scraper.build_ai_context(
            extract_key_values_from_html(""),
            ParsedHTML(key_values={}, text=notice_text, rows=[]),
            "ip",
            "309329493",
            project_name="某公司知识产权项目",
        )

        self.assertIn(late_sentence, context.notice_text)
        hints = context.html_key_values.get("_activity_time_hints", "")
        self.assertIn("2026年9月18日10时", hints)

    def test_v2_resource_payload_keeps_media_for_item_resources(self):
        payload = scraper.build_resource_payload(
            [{"attachmentName": "评估报告.zip", "attachmentAddress": "https://storage.jd.com/report.zip"}],
            {"data": {"imageVideoArea": {"imageList": [{"imagePath": "jfs/t1/example.jpg"}]}}},
        )

        self.assertEqual(len(payload["files"]), 1)
        self.assertEqual(len(payload["media"]), 1)
        rows = jd_mysql_store.build_v2_resource_rows(
            item_id=1,
            attachments_json=payload,
            asset_group="real_estate",
        )
        resource_types = {row["resource_type"] for row in rows}
        self.assertIn("attachment", resource_types)
        self.assertIn("image", resource_types)

    def test_v2_other_type_uses_notice_text_and_ai_summary_when_detail_is_empty(self):
        scraper.ai_extractor = FakeBatchAIExtractor(
            {
                "extracted_summary": {
                    "value": "沈阳金杯方家栏厂区部分资产，包含房屋、土地及附属设施，详情以公告和评估报告为准。",
                    "source_text": "拍卖标的：沈阳金杯车辆制造有限公司方家栏厂区部分资产。",
                }
            }
        )
        ...
        values, results, _ = extract_special_values(
            ...
            parsed=extract_key_values_from_html(""),
            notice_parsed=extract_key_values_from_html(
                "<p>拍卖标的：沈阳金杯车辆制造有限公司方家栏厂区部分资产。</p>"
                "<p>标的物以现状为准，相关瑕疵及税费由买受人自行核实。</p>"
            ),
            core={"data": {"basicData": {"title": "方家栏厂区部分资产"}}},
            paimai_id="310829589",
            attachments={"files": [], "media": []},
        )

        self.assertIn("方家栏厂区部分资产", values["raw_detail_text"])
        self.assertIn("方家栏厂区部分资产", values["extracted_summary"])
        self.assertEqual(results["extracted_summary"]["source_payload_type"], "ai_extraction")

    def test_v2_ip_images_enqueue_async_ocr_when_details_are_not_available(self):
        old_ai = scraper.ai_extractor
        scraper.ai_extractor = None
        try:
            values, results, details = extract_special_values(
                asset_group="ip",
                parsed=ParsedHTML(
                    key_values={},
                    text="标的名称：某公司名下软件著作权及专利权。",
                    rows=[],
                    image_urls=["jfs/t1/ip-table.jpg"],
                ),
                notice_parsed=extract_key_values_from_html(""),
                core={"data": {"basicData": {"title": "某公司名下软件著作权及专利权"}}},
                paimai_id="ip-ocr-test",
                attachments={"files": [], "media": []},
            )
        finally:
            scraper.ai_extractor = old_ai

        self.assertEqual(details, [])
        self.assertIn("_ocr_retry_task", results)
        self.assertEqual(results["_ocr_retry_task"]["task_type"], "ip_image_details")
        self.assertIn("https://img30.360buyimg.com/popWaterMark/jfs/t1/ip-table.jpg", results["_ocr_retry_task"]["image_urls"])

    def test_v2_ip_images_do_not_enqueue_ocr_when_ai_returns_item_details(self):
        old_ai = scraper.ai_extractor
        scraper.ai_extractor = FakeBatchAIExtractor(
            {
                "ip_details": [
                    {
                        "sequence_no": "1",
                        "ip_name": "智能水刀切割特殊路径生成系统",
                        "certificate_no": "软著登字第13029563号",
                        "ip_type": "软件著作权",
                        "source_excerpt": "软件名称：智能水刀切割特殊路径生成系统",
                    }
                ]
            }
        )
        try:
            values, results, details = extract_special_values(
                ...
                notice_parsed=extract_key_values_from_html(""),
                core={"data": {"basicData": {"title": "某公司名下软件著作权及专利权"}}},
                paimai_id="ip-no-ocr-test",
                attachments={"files": [], "media": []},
            )
        finally:
            scraper.ai_extractor = old_ai

        self.assertEqual(values["ip_count"], "1")
        self.assertEqual(len(details), 1)
        self.assertNotIn("_ocr_retry_task", results)

    def test_v2_core_normalizers_are_not_duplicated(self):
        source = Path(scraper.__file__).read_text(encoding="utf-8")

        self.assertEqual(source.count("\ndef parse_datetime_value("), 1)
        self.assertEqual(source.count("\ndef normalize_area_value("), 1)
        self.assertEqual(source.count("\nAREA_UNIT_RE ="), 1)

    def test_attachment_name_and_url_supports_common_download_aliases(self):
        name, url = scraper.attachment_name_and_url(
            {
                "fileName": "评估报告.pdf",
                "downloadUrl": "https://storage.jd.com/report.pdf",
            }
        )

        self.assertEqual(name, "评估报告.pdf")
        self.assertEqual(url, "https://storage.jd.com/report.pdf")
