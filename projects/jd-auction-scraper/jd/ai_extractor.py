...
class AIFieldExtractor:
    ...
    def __init__(
        ...
        self.min_interval = 0.1  # 最小请求间隔（秒），对应配置中的 10 QPS

        # 初始化客户端
        if api_key:
            self._init_client(provider, api_key, base_url, timeout, max_retries)
...
{json.dumps(context.html_key_values, ensure_ascii=False, indent=2)[:1000]}

2. 标的详情文本:
{context.detail_text[:5000]}

3. 竞买须知文本:
{context.notice_text[:4000]}
...
7. 对 special_notice：只有页面明确出现“特别告知/特别提示/特别提醒/特别说明/重要提示/注意事项/重大事项/瑕疵说明/风险提示”等标题时才提取该标题下内容；普通风险描述或“其他说明”不要作为特别告知。
...
【数据源】
1. HTML 表格键值对（已提取的结构化数据）:
{json.dumps(context.html_key_values, ensure_ascii=False, indent=2)[:1500]}

2. 标的详情文本:
{context.detail_text[:8000]}

3. 竞买须知文本:
{context.notice_text[:5000]}
...
{{
    ...
    def extract_ip_details_from_images(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                cfg = get_config()
                vision_model = getattr(cfg.ai, "vision_model", "") or (
...
【图片数量】{len(image_urls)}
【文字上下文】
{context.detail_text[:1000]}
{context.notice_text[:1000]}
