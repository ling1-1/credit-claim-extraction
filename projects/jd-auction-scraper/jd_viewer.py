...
<body>
  <div class="topbar">
    ...
    <div class="muted">SQLite 本地只读查看</div>
  </div>
...
def render_detail(source: DataSource, paimai_id: str) -> bytes:
    ...
    <section class="section">
      <h2>共有字段</h2>
      {render_fields(detail['common_fields'])}
    </section>
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mysql-host", default=os.getenv("JD_MYSQL_HOST", "127.0.0.1"), help="MySQL 主机")
    parser.add_argument("--mysql-port", type=int, default=int(os.getenv("JD_MYSQL_PORT", "3306")), help="MySQL 端口")
    parser.add_argument("--mysql-user", default=os.getenv("JD_MYSQL_USER", ""), help="MySQL 用户")
    parser.add_argument("--mysql-password", default=os.getenv("JD_MYSQL_PASSWORD", ""), help="MySQL 密码")
    parser.add_argument("--mysql-database", default=os.getenv("JD_MYSQL_DATABASE", "auction_data"), help="MySQL 数据库")
