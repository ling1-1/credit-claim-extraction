...
def init_ai_extractor(model: str = "", api_key: str = "", base_url: str = "") -> None:
    """初始化 AI 提取器"""
    global ai_extractor
    if api_key:
        config = {
        ...
        ai_extractor = create_ai_extractor(config)
    else:
        logger.warning("ai_extractor_disabled", "AI 提取器未配置 API Key，仅使用规则提取")
...
AI_DETAIL_TEXT_LIMIT = 5000
AI_NOTICE_TEXT_LIMIT = 4000
...
def build_ai_context(parsed: ParsedHTML, notice_parsed: ParsedHTML, asset_group: str, paimai_id: str) -> AIExtractionContext:
    key_values: Dict[str, str] = {}
    image_urls: List[str] = []
    ...
    if image_urls:
        key_values["_image_urls_json"] = safe_json_dumps(image_urls[:20])
    key_values.update(parsed.key_values)
    if parsed.rows:
        key_values["_detail_table_rows_json"] = safe_json_dumps(parsed.rows[:120])
    if notice_parsed.rows:
        key_values["_notice_table_rows_json"] = safe_json_dumps(notice_parsed.rows[:80])
...
def ai_batch_extract_fields(
    ...
    *,
    ...
    if ai_extractor is None or not fields:
        return {}
    context = build_ai_context(parsed, notice_parsed, asset_group, paimai_id)
    try:
...
def normalize_area_value(value: Any, source_text: Any = None) -> Any:
    ...
    for match in re.finditer(r"(\d[\d,]*(?:\.\d+)?)\s*(平方米|平方|平米|㎡|m2|m²|亩|公顷)", source):
...
def normalize_area_value(value: Any, source_text: Any = None) -> Any:
    ...
    if loose_unit:
        return f"{text}{loose_unit.group(1)}"
    return text
    ...
    *,
    ...
    if asset_group == "other":
        ...
        results["raw_detail_text"] = field_result_value(
            parsed.text, "description_html", "text", parsed.text[:300], "html_text"
        )
        results["raw_table_pairs_json"] = field_result_value(
            parsed.key_values, "description_html", "table_pairs", method="html_table"
        )
...
class JDAuctionScraper:
    ...
    def crawl_sample(
        self, *, per_category_limit: int, output_dir: Path, categories: set[str] | None = None
    ) -> Dict[str, Any]:
        """采集样本数据"""
        self.db.init_schema()
    ...
    def _crawl_one(self, batch_id: str, category: JDCategory, list_item: Dict[str, Any], paimai_id: str) -> None:
        ...
        special_values, special_results, asset_details = extract_special_values(
            ...
            paimai_id=paimai_id,
            attachments={
                "files": bundle.get("attachments") or [],
                "media": extract_media(bundle.get("core") or {}),
            },
        )
...
def main() -> None:
    ...
    if args.command == "crawl":
        ...
        # 初始化 AI 提取器（如果配置了）
        if args.ai_api_key or cfg.ai.api_key:
            init_ai_extractor(
                model=args.ai_model or cfg.ai.model,
                api_key=args.ai_api_key or cfg.ai.api_key,
                base_url=args.ai_base_url or cfg.ai.base_url,
            )
